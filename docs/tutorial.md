...
created. Then, we need to install `fulmo-cookiecutter-poetry` with the
following command:

``` bash
pip install fulmo-cookiecutter-poetry
```
...
that you also gave the Github repository and `<github_username>`
...
you should see a notification saying `` Your site is ready to be published at https://<github_username>.github.io/<project_name>/``.
